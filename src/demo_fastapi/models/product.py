...
class Product(Base):
    ...
    id: Mapped[int] = mapped_column(
        BigInteger, primary_key=True, autoincrement=True, comment="内部ID"
    )  # 内部ID
    product_id: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="商品ID"
    )  # required: gap, jcpenney, target
    source: Mapped[Literal["gap", "target", "next", "jcpenney", "other"]] = (
        mapped_column(String(64), default="other", nullable=True, comment="数据来源")
    )  # required: gap jcpenney, target
    ...
    brand: Mapped[str | None] = mapped_column(
        String(64), comment="品牌"
    )  # required: gap, jcpenney, target
    product_url: Mapped[str | None] = mapped_column(
        String(1024), comment="商品链接"
    )  # required: gap, jcpenney, target
    ...
    attributes_raw: Mapped[dict | list | None] = mapped_column(
        JSON, comment="原始属性"
    )  # optional: gap专用
    category: Mapped[Literal["women", "men", "girls", "boys", "other"] | None] = (
        mapped_column(String(256), comment="商品类别")
    ...
    current_version: Mapped[str | None] = mapped_column(
        String(32), default="0", comment="当前版本"
    )
    is_review_analyzed: Mapped[bool | None] = mapped_column(
        Boolean, default=False, comment="是否已分析"
    )
    review_analyses: Mapped[list[dict] | None] = mapped_column(
        JSON, comment="评论分析结果汇总"
    )
    extra_review_analyses: Mapped[list[dict] | None] = mapped_column(
        JSON, comment="额外评论分析结果汇总"
    )
    extra_metrics: Mapped[list | dict | None] = mapped_column(
        JSON, comment="额外评论指标"
    )
    review_statistics: Mapped[dict | None] = mapped_column(JSON, comment="评论分析统计")
    extra_review_statistics: Mapped[dict | None] = mapped_column(
        JSON, comment="额外评论分析统计"
    )
    ...
    is_deleted: Mapped[bool | None] = mapped_column(
        Boolean, default=False, comment="软删除"
    )
...
class ProductTranslation(Base):
    ...
    language_code: Mapped[Literal["zh", "en"]] = mapped_column(
        String(16), nullable=False, default="zh", comment="语言"
    )
    product_id: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="商品ID"
    )  # required: gap, jcpenney, target
...
class ProductDetail(Base):
    ...
    id: Mapped[int] = mapped_column(
        ForeignKey("product.id"), primary_key=True, comment="内部ID"
    )
    product_id: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="商品ID"
    )  # required: gap, jcpenney, target
...
class ProductSKU(Base):
    ...
    __table_args__ = (
        ...
        Index("ix_product_id", "product_id"),
        UniqueConstraint(
            "product_id", "sku_id", "source", name="uq_product_id_sku_id_source"
        ),
        {"comment": "商品SKU"},
    )
    id: Mapped[int] = mapped_column(
    ...
    model_image_urls: Mapped[list[str] | None] = mapped_column(
        JSON, comment="模特图片链接列表"
    )
    outer_model_image_urls: Mapped[list[str] | None] = mapped_column(
        JSON, comment="外部模特图片链接列表"
    )
...
class ProductSKUDetail(Base):
    ...
    id: Mapped[int] = mapped_column(
        ForeignKey("product_sku.id"), primary_key=True, comment="内部ID"
    )
    source: Mapped[Literal["gap", "target", "next", "jcpenney", "other"]] = (
        mapped_column(String(64), default="other", nullable=True, comment="数据来源")
    )  # required: gap, jcpenney, next, target
...
class ProductReview(Base):
    ...
    __table_args__ = (
        ...
        UniqueConstraint(
            "review_id", "product_id", "source", name="uq_review_id_product_id_source"
        ),
        {"comment": "商品评论"},
    )
    id: Mapped[int] = mapped_column(BigInteger, primary_key=True, autoincrement=True)
    review_id: Mapped[str | None] = mapped_column(
        String(64), comment="源评论ID"
    ...
    outer_photos: Mapped[list[str] | None] = mapped_column(
        JSON, comment="评论外部数据源"
    )  # optional: target
    nickname: Mapped[str | None] = mapped_column(
        String(64), comment="昵称"
    )  # required: gap, jcpenney, next
    helpful_votes: Mapped[int | None] = mapped_column(
        Integer, default=0, comment="按顶票数"
    )  # required: gap, jcpenney
    not_helpful_votes: Mapped[int | None] = mapped_column(
        Integer, comment="按踩票数"
    )  # required: gap, jcpenney
    # helpful_score: Mapped[float | None] = mapped_column(Numeric(6, 1), comment="有用评分")  # optional: gap
    is_deleted: Mapped[bool | None] = mapped_column(
        Boolean, default=False, nullable=True, comment="软删除"
    )
    gathered_at: Mapped[datetime | None] = mapped_column(
        TIMESTAMP(timezone=True),
        default=func.now(),
...
class ProductReviewTranslation(Base):
    ...
    sku_id: Mapped[str | None] = mapped_column(
        String(64), comment="SKU ID, 部分系统支持sku_id区分"
    )  # optional: gap, next
    language_code: Mapped[Literal["zh", "en"]] = mapped_column(
        String(16), nullable=False, default="zh", comment="语言代码"
    )
...
class ProductReviewAnalysis(Base):
    ...
    version_id: Mapped[str | None] = mapped_column(
        String(32), default="0", comment="版本ID"
    )
    source: Mapped[Literal["gap", "target", "next", "jcpenney", "other"]] = (
        mapped_column(String(64), default="other", nullable=True, comment="数据来源")
    )  # required: gap, jcpenney, next
    ...
    quality: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品质量指标"
    )
    warmth: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品保暖性指标"
    )
    comfort: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品舒适度指标"
    )
    softness: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品柔软性指标"
    )
    preference: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品偏好指标"
    )
    repurchase_intent: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品回购意愿指标"
    )
    appearance: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品外观指标"
    )
    fit: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="商品合身度指标"
    )
    extra_metrics: Mapped[dict | list | None] = mapped_column(
        JSON, comment="额外评论指标"
    )
...
class ReviewAnalysisExtraMetric(Base):
    ...
    value: Mapped[float | None] = mapped_column(
        Numeric(3, 1), default=0, comment="指标得分"
    )
    created_at: Mapped[datetime | None] = mapped_column(
        TIMESTAMP(timezone=True),
        default=func.now(),
