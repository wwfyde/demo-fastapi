...
class LLM(Base):
    ...
    name: Mapped[str] = mapped_column(
        String(64), unique=True, index=True, nullable=False, comment="模型名称"
    )
    ...
    access_key: Mapped[str | None] = mapped_column(
        String(128), comment="Access Key for AWS bedrock"
    )
    secret_key: Mapped[str | None] = mapped_column(
        String(128), nullable=True, comment="Secret Key for AWS bedrock"
    )
    model: Mapped[str | None] = mapped_column(
        String(64), nullable=True, comment="默认模型"
    )
    model_extra: Mapped[list[str] | None] = mapped_column(
        JSON, nullable=True, comment="模型列表"
    )
    config: Mapped[dict | None] = mapped_column(JSON, nullable=True, comment="配置列表")
    status: Mapped[bool | None] = mapped_column(
        Boolean, nullable=True, default=True, comment="渠道状态"
    )
    is_deleted: Mapped[bool] = mapped_column(
        Boolean, nullable=True, default=False, comment="是否删除"
    )
