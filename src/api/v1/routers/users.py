...
@router.post("/users/", response_model=schemas.user.UserModel)
...
@router.get("/users/", response_model=list[schemas.user.UserModel])
def read_users(skip: int = 0, limit: int = 100, db: Session = Depends(get_db)):
    users = crud.crud_user.get_users(db, skip=skip, limit=limit)
    return users


@router.get("/users/{user_id: int}", response_model=schemas.user.UserModel)
